        ...
        <div >
            {Data?.map((title) => {
                return <div className={`${font}`}>
                    ...
                    <div>{title.sub_fields && <Columns Data={title.sub_fields} font={'text-lg'} border={'border-l-2 border-r-2'}/>}</div>
                    ...
                    <div className=' border-2 flex'>
                        ...
                        <div className='flex'>{TableData.column_data.map((item) => (
                            item.form_field_id === title.form_field_id ? item.data.map((v) => (<div className=' border-l-2 w-28 p-2'>{v.value}</div>)) : ''
                        ))}
                        </div>
                    </div>
                </div>
